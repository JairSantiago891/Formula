...
namespace Homer_MVC.Controllers
{
    public class CatalogosController : Controller
    {
        ...
        public ActionResult Detalle(Guid valor)
        {
            ...
            var v = ApiConnect.GetEncoder(valor);

            if (v == null)
            {
                Session["Encoder"] = null;
                v = new EncoderIp(valor);
            }
            else
            {
                Session["Encoder"] = "G"; ;
        ...
        [HttpGet]
        public ActionResult Encoder(EncoderIp v)
        {
            var loca = Request.Form;
            Session["Status"] = "";
            return PartialView("Create", new EncoderIp("Encoder"));
        }

        [HttpPost] 
        public ActionResult Create(EncoderIp v)
        {
            var ret001 = (EncoderIp)Session["IdEncoder"];
            if (ret001 != null)
            {
                v.PlazaId = ret001.PlazaId;
            }
            ApiConnect.AddEncoder(v);

            return RedirectToAction("Index", "Catalogos", null);
        }
